...
class Animal {
    private: //Can't be accessed outside the class
        ...
        int weight;
        std::string name;
        static int count; //static means this variable is shared by all instances of the class
    public: //Can be accessed outside the class
        ...
        static void setCount(int c) { count = c; } //Since it uses a static variable, it must be static
...
class Dog : public Animal {
    ...
    public:
        ...
        Dog(int h, int w, std::string n) : Animal(h, w, n) {} //To set the dog's animal vars, declare them in the initializer list
...
class Cat : public Animal {
    ...
    public:
        Cat(int h, int w, std::string n, bool litterTrained) : Animal(h, w, n) {
            this->litterTrained = litterTrained;//This refers to the current object of the class using the function
        }
        void speak() const { //This function is a const since it doesn't modify anything
            std::string sentence = sound;
            sentence += ". I am ";
            sentence += (litterTrained ? "" : "not ");   
            sentence += "litter trained.";
            std::cout << sentence << std::endl;
        }
        // Friend function - not implemented in class, but elsewhere
        //Reference original object to directly modify private data members
        friend void setLitterTrained(Cat &c); //friend function so it can access private data members
};
