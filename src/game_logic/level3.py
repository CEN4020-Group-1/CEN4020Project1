...
class Level3Logic:
    ...
    def _is_valid_move(self, row, col):
        ...
        #Check if cell aligns with ring row
        #Check if cell aligns with ring col
        #Check if cell aligns with ring diagonal
        #Check if cell aligns with ring antidiagonal
    ...
    def _get_valid_cells(self):   #get list of all valid cells for current move
        ...
        elif self._is_on_diagonal(outer_x, outer_y):#Check if outer ring pos is diagonal
            for offset in range(5):
                is_valid, _ = self._is_valid_move(offset, offset)
                if is_valid(offset, offset):
                    valid.append((offset, offset))
                
        elif self._is_on_anti_diagonal(outer_x, outer_y): #Check if it's anti diagonal
            for row in range(5):
